...
#
# 2023-07-07: These are utility functions for type checking,
#   copied from the forcats source file of the same name.
#
# Dependencies
# ============
#
...
.rlang_types_check_number <- function(x,
  ...
  if (!missing(x)) {
    ...
    if (allow_na && (identical(x, NA) ||
                     identical(x, rlang::na_dbl) ||
                     identical(x, rlang::na_int))) {
      return(invisible(NULL))
    }
  }
...
is_number <- function(x,
  ...
  if (!allow_decimal && !rlang::is_integerish(x)) {
    return(FALSE)
  }
