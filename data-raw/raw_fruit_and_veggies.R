...
set.seed(42)
fruit_lbl <- labelled(sample(as.double(1:5), 50, TRUE, prob=c(3,3,2,5,1)),
              labels = c(Apple = 1, Banana = 2, Lime = 3, Orange = 4, Peach = 5))

fruit_lbl_int <- labelled(as.integer(fruit_lbl),
                   labels = c(Apple = 1, Banana = 2, Lime = 3, Orange = 4, Peach = 5))

fruit_lbl_chr <- labelled(letters[fruit_lbl],
...
veggies <- labelled(primes[ix_plain])
labs <- primes[1:5]
names(labs) = plains
val_labels(veggies) <- labs
var_label(veggies) <- plain.var_label
veggies |> lbl_count()
...
exotic_veggies <- labelled(vals_exotic)
labs <- primes[1:13]
names(labs) = exotics
val_labels(exotic_veggies) <- labs
var_label(exotic_veggies) <- exotic.var_label
